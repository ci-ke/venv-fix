...
def fix_activate(targetenv_path):
    ...
    for line in activate_file:
        ...
        if ') \" != x ] ; then' in line:
            line = '    if [ \"x('+env_name+') \" != x ] ; then\n'
        if ') ${PS1:-}\"' in line:
            line = '    PS1=\"('+env_name+') ${PS1:-}\"\n'
...
def fix_activate_ps1(targetenv_path):
    ...
    for line in activate_file:
        ...
        if 'Write-Host -NoNewline -ForegroundColor Green \'(' in line:
            line = 'Write-Host -NoNewline -ForegroundColor Green \'('+env_name+') \'\n'
        modified_activate_file.write(line)
    activate_file.close()
    modified_activate_file.close()
    os.remove(activate_path)
    ...
    for scriptexe_name in scriptexe_names:
        if scriptexe_name.split('.')[-1] == 'exe' and scriptexe_name != 'python.exe' and scriptexe_name != 'pythonw.exe' and scriptexe_name != modified_python_name:
            ...
            if modified_python_name:
                offset_python = offset_python = rawcontent.find(bytes(modified_python_name, 'ascii')+b'\x0a\x0d\x0aPK')
            else:
            ...
            if modified_python_name:
                ...
                replace_python_path = ('#!'+targetenv_path+'\\Scripts\\'+modified_python_name).encode('ascii')
            else:
                end_offset = offset_python+10
                replace_python_path = ('#!'+targetenv_path+'\\Scripts\\python.exe').encode('ascii')
            start_offset = offset_python
            while rawcontent[start_offset:start_offset+2] != b'#!':
                start_offset -= 1
            newcontent = rawcontent[:start_offset]+replace_python_path+rawcontent[end_offset:]
...
if __name__ == "__main__":
    if len(sys.argv) != 2 and len(sys.argv) != 3:
        print('Usage: python '+sys.argv[0]+' <VENV_PATH> (modified_python_name)')
        exit()
